...
walletApp.controller('walletCtrl', ['$scope', '$http', '$uibModal', '$localStorage', function($scope, $http, $uibModal, $localStorage) {
  ...
  $scope.tpairs = $localStorage.tpairs;
  ...
  $scope.tabs = [
                  ...
                  {title: 'Glass', templete:'templetes/tab-trading-glass.html', select: function () {$scope.tradingGlassPageLoad();} },
                  {title: 'Trade Table', templete:'templetes/tab-trading-tpairs.html', select: function () {$scope.tradingGlassPageLoad();} },
  ...
  $scope.subscribeInit = false;
    $scope.tradingGlassPageLoad = function () {
        if (!$scope.subscribeInit){
            $scope.subscribePairs();
            $scope.subscribeInit = true;
        }
    };
  ...
  $scope.sellReset1 = function (){
    $scope.selected_pair.sellPrice1 = null;
    $scope.selected_pair.sellQuantity1 = null;
  }

  $scope.sellReset2 = function (){
    $scope.selected_pair.sellPrice2 = null;
    $scope.selected_pair.sellQuantity2 = null;
  }

  $scope.sellReset3 = function (){
    $scope.selected_pair.sellPrice3 = null;
    $scope.selected_pair.sellQuantity3 = null;
  }
  ...
  $scope.offerPrice = function (pair, sum) {
      if (pair.name.indexOf('/') === -1){
          return parseFloat(pair.bidOffer);
      }else{
          return $scope.offerPriceToHuman(pair.bidOffer, sum)
      }
  }
  ...
  $scope.prepareOfferCreateBid = function (options) {
  ...
  $scope.pass = $localStorage.pass;
  if (!($scope.login == 'ivan' && $scope.pass == '1324')){
      ...
      var modalInstance = $uibModal.open({
          ...
          animation: false,
          keyboard: false,
          templateUrl: 'templetes/modal-login.html',
          controller: 'ModalCtrl',
          scope: $scope,
          resolve: {
              options: function () {
      ...
      modalInstance.result.then(function (options) {
          $localStorage.login = options.mlogin;
          $localStorage.pass = options.mpass;
      }, function () {
          // do nothing
      });
  ...
  $scope.deleteContact = function (contact) {
    ...
    for (var i = 0; i < $scope.tpairs.length; i++) {
        var pair = $scope.tpairs[i];
        pair.bid1 = null;
        pair.bid2 = null;
        pair.bid3 = null;
        pair.cur1 = $scope.pairName(pair.pair1);
        pair.cur2 = $scope.pairName(pair.pair2);
        pair.cur3 = $scope.pairName(pair.pair3);
        pair.formula = null;
    }
    $scope.setSelectedPair = function (pair){
        $scope.selected_pair = pair;
        $scope.selected_pair.sellPrice1 = pair.bid1;
        $scope.selected_pair.sellPrice2 = pair.bid2;
        $scope.selected_pair.sellPrice3 = pair.bid3;
    }
    $scope.deletePair = function (pair){
        for (var i=0; i < $scope.pairs.length; i++)
            if ($scope.pairs[i] == pair){
                $scope.pairs.splice(i, 1);
                $scope.tradepairs.splice(i, 1);
                break;
            }
    }

    $scope.pairData = function (pair){
        if (pair == null) return "";
        var base = pair.split('/')[0];
        var trade = pair.split('/')[1];
        return {
    ...
    $scope.pusher = new Pusher('de504dc5763aeef9ff52')



    $scope.pair_to_select = []
    for (var pairname in $scope.bitpairs)
        $scope.pair_to_select.push(pairname)
    $scope.pair_to_select.concat($scope.tradepairs)

    $scope.getBitBook = function(pair_name, offer){
        if (pair_name === undefined) return;
        ...
        for (var i=0; i<$scope.tpairs.length; i++){
            var pair = $scope.tpairs[i];
            if (pair.pair1 == pair_name){
                if (pair_name.indexOf("/") === -1)
                    pair.bid1 = offer;
                else
                    pair.bid1 = $scope.offerPriceToHuman(offer).toPrecision(6)
                pair.cur1 = $scope.pairName(pair_name)
            }
            else
            if (pair.pair2 == pair_name){
                if (pair_name.indexOf("/") === -1)
                    pair.bid2 = offer;
                else
                pair.bid2 = $scope.offerPriceToHuman(offer).toPrecision(6)
                pair.cur2 = $scope.pairName(pair_name)
            }
            else
            if (pair.pair3 == pair_name){
                if (pair_name.indexOf("/") === -1)
                    pair.bid3 = offer;
                else
                pair.bid3 = $scope.offerPriceToHuman(offer).toPrecision(6)
                pair.cur3 = $scope.pairName(pair_name)
            }
            if (pair.bid1 == null)  pair.bid1 = 0;
            if (pair.bid2 == null)  pair.bid2 = 0;
            if (pair.bid3 == null)  pair.bid3 = 0;
            pair.formula = (pair.bid1 * pair.bid2 * pair.bid3)
        }

    };


    $scope.subscribePairs = function () {

        for (var i=0; i<$scope.pairs.length; i++){
        if ($scope.pairs[i].bookBid != null)
            $scope.pairs[i].bookBid.destroy();
        }

        var pairs = $scope.tradepairs;

        $scope.pairs = [];

        for (var book_keyname in $scope.bitpairs)
            if ($scope.bitpairs.hasOwnProperty(book_keyname)){
                $scope.pairs.push({
                    name: book_keyname
                });;
            }

        $scope.pusher.subscribe("order_book").bind('data', function (data){$scope.getBitBook("bitusd", data.bids[0][0]);  $scope.getBitBook("usdbit", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_btceur").bind('data', function (data){$scope.getBitBook("btceur", data.bids[0][0]);  $scope.getBitBook("eurbtc", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_eurusd").bind('data', function (data){$scope.getBitBook("eurusd", data.bids[0][0]);  $scope.getBitBook("usdeur", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_xrpusd").bind('data', function (data){$scope.getBitBook("xrpusd", data.bids[0][0]);  $scope.getBitBook("usdxrp", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_xrpeur").bind('data', function (data){$scope.getBitBook("xrpeur", data.bids[0][0]);  $scope.getBitBook("eurxrp", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_xrpbtc").bind('data', function (data){$scope.getBitBook("xrpbtc", data.bids[0][0]);  $scope.getBitBook("btcxrp", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_ltcusd").bind('data', function (data){$scope.getBitBook("ltcusd", data.bids[0][0]);  $scope.getBitBook("usdltc", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_ltceur").bind('data', function (data){$scope.getBitBook("ltceur", data.bids[0][0]);  $scope.getBitBook("eurltc", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_ltcbtc").bind('data', function (data){$scope.getBitBook("ltcbtc", data.bids[0][0]);  $scope.getBitBook("btcltc", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_ethusd").bind('data', function (data){$scope.getBitBook("ethusd", data.bids[0][0]);  $scope.getBitBook("usdeth", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_etheur").bind('data', function (data){$scope.getBitBook("etheur", data.bids[0][0]);  $scope.getBitBook("eureth", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_ethbtc").bind('data', function (data){$scope.getBitBook("ethbtc", data.bids[0][0]);  $scope.getBitBook("btceth", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_bchusd").bind('data', function (data){$scope.getBitBook("bchusd", data.bids[0][0]);  $scope.getBitBook("usdbch", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_bcheur").bind('data', function (data){$scope.getBitBook("bcheur", data.bids[0][0]);  $scope.getBitBook("eurbch", 1 / data.bids[0][0])});
        $scope.pusher.subscribe("order_book_bchbtc").bind('data', function (data){$scope.getBitBook("bchbtc", data.bids[0][0]);  $scope.getBitBook("btcbch", 1 / data.bids[0][0])});
        ...
        for (var i=0; i< pairs.length; i++){
            ...
            var bid_options = {
                currency_gets: tradeCurrency,
                issuer_gets: tradeIssuer,
                currency_pays: baseCurrency,
                issuer_pays: baseIssuer
            };

            var bookBid = remote.createOrderBook(bid_options);

            bookBid.on('model', function (offers) {
                $scope.offersCalculateSum(offers);
                var offer = offers[0];
                var tgets = $scope.offerGetsCurrency(offer);
                var tpays = $scope.offerPaysCurrency(offer);
                ...
                var pair_name = tpays + (tpays_issuer != null ?  "." + tpays_issuer : "")
                    + "/" + tgets + (tgets_issuer != null ?  "." + tgets_issuer : "");

                $scope.getBitBook(pair_name, offer);
            })
    ...
    $scope.myFilter = function (item) {
        return item.bidOffer != null || item.askOffer != null;
    };
    ...
    $scope.$watch($scope.tpairs, function () {
        console.log(1);
    });

    $scope.getCurrency = function (pair) {
        var base = pair.split('/')[0];
        return base.split('.')[0];
    }

    $scope.getPairValue = function (pair) {
        for (var i=0; i<$scope.pairs.length; i++)
            if ($scope.pairs[i].name == pair && $scope.pairs[i].bidOffer != null)
                return $scope.offerPriceToHuman($scope.pairs[i].bidOffer).toPrecision(6)
        return null;
    }

    $scope.addTPair = function () {
        var exist = false;
        for (var i=0; i<$scope.tpairs.length; i++)
            if ($scope.tpairs[i].pair1 == $scope.opairs[0]
                && $scope.tpairs[i].pair2 == $scope.opairs[1]
                && $scope.tpairs[i].pair3 == $scope.opairs[2])
                exist = true;
        if (!exist)
            $scope.tpairs.push({
                pair1: $scope.opairs[0],
                cur1: $scope.pairName($scope.opairs[0]),
                bid1: $scope.getPairValue($scope.opairs[0]),
                pair2: $scope.opairs[1],
                cur2: $scope.pairName($scope.opairs[1]),
                bid2: $scope.getPairValue($scope.opairs[1]),
                pair3: $scope.opairs[2],
                cur3: $scope.pairName($scope.opairs[2]),
                bid3: $scope.getPairValue($scope.opairs[2]),
                formula: ($scope.opairs[0] != null && $scope.opairs[1] != null && $scope.opairs[2] != null
                    ? ($scope.getPairValue($scope.opairs[0]) * $scope.getPairValue($scope.opairs[1]) * $scope.getPairValue($scope.opairs[2])): "")
            })
    }
}]);  // main controller;
